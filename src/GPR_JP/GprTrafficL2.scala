...
/**
  * GPR trafficL2 calculates the trafficL2 aggregate metrics
  *
...
object GprTrafficL2 {
  ...
  def buildAggGblTrafficL2(startDate : String, endDate : String)={
    ...
    val stgGblTrafficL2Qry =
      s"""
         ...
         |       , CASE WHEN a.grt_l1_cat_name IS NULL THEN 'unknown' ELSE a.grt_l1_cat_name END AS grt_l1_cat_name
         |       , CASE WHEN a.grt_l2_cat_name IS NULL THEN 'unknown' ELSE a.grt_l2_cat_name END AS grt_l2_cat_name
         |       , a.brand
    ...
    val stgGblTrafficL2Df = sql(stgGblTrafficL2Qry)

    stgGblTrafficL2Df.createOrReplaceTempView("stg_gbl_traffic_l2")

    val aggGblTrafficL2Qry =
      s"""
         |SELECT
    ...
    val aggGblTrafficL2 = sql(aggGblTrafficL2Qry).distinct

    println ("Writing into Hdfs file")

    //Insert data into agg_gbl_traffic_l1 partition table
    aggGblTrafficL2.coalesce(2).write.mode("overwrite").insertInto("$tgtSchema.agg_gbl_traffic_l2".replace("$tgtSchema",tgtSchema))
